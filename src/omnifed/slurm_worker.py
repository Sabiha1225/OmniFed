...
# This separator is used only in temporary gRPC transport keys.
# It should not occur in normal PyTorch parameter names.
_TENSOR_SLICE_SEPARATOR = ".__omnifed_slice__."


def _make_slice_key(name: str, start: int) -> str:
    return f"{name}{_TENSOR_SLICE_SEPARATOR}{start}"


def _parse_slice_key(key: str) -> tuple[str, int] | None:
...
def iter_state_chunks(state_dict, chunk_size_mb: int):
    ...
    for name in sorted(state_dict):
        ...
        for start in range(0, flat_tensor.numel(), elements_per_slice):
            ...
            slice_key = _make_slice_key(name, start)
...
class StateChunkAssembler:
    ...
    def add_chunk(self, transport_chunk):
        for transport_key, value in transport_chunk.items():
            parsed = _parse_slice_key(transport_key)
...
def run_client_global_communication(
    ...
    # aggregated_state = {}
    assembler = StateChunkAssembler(local_state)
...
def run_federated_server(cfg, role: SlurmRole) -> None:
    ...
    for round_id in range(int(cfg.global_rounds)):
        ...
        # new_global_state = {}
        assembler = StateChunkAssembler(global_state)
